...
export const reservationRepository = {
  ...
  upsertPayment(reservationId: string, providerPaymentId: string, amount: number) {
    return prisma.payment.upsert({
      where: { reservationId },
      create: { reservationId, providerPaymentId, amount, status: 'PENDING' },
      update: { providerPaymentId, amount, status: 'PENDING' },
    })
  },

  setPaymentStatus(reservationId: string, status: 'PENDING' | 'SUCCEEDED' | 'CANCELED') {
    return prisma.payment.update({ where: { reservationId }, data: { status } })
  },
